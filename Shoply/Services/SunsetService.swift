...
class SunsetService {
    ...
    /// Détermine si c'est le jour ou la nuit basé sur le coucher de soleil
    func isDaytime(latitude: Double, longitude: Double, currentTime: Date = Date()) -> Bool {
        guard let sunset = calculateSunset(latitude: latitude, longitude: longitude, date: currentTime) else {
            // Fallback: utiliser l'heure pour déterminer jour/nuit
            let hour = Calendar.current.component(.hour, from: currentTime)
            return hour >= 6 && hour < 20
        }
        
        // Si l'heure actuelle est avant le coucher de soleil, c'est le jour
